...
fn render_report(args: ReportArgs) -> Result<()> {
    ...
    let output = args.output.unwrap_or_else(|| {
        report_site()
            .default_output_for_bundle(&args.bundle_dir)
    ...
    {
        report_site().update_indexes_for_file(&output)?;
    } else {
        report_site().update_latest_link(&args.bundle_dir)?;
    }
...
fn write_artifact_bundle(
    ...
    if let Some(scenario) = scenario {
        ...
        if let Some(report_site_output) = report_site().default_output_for_bundle(path) {
            write_run_report(
                &report_site_output,
                scenario,
                controller_spec,
            ...
            report_site().update_indexes_for_file(&report_site_output)?;
        }
        report_site().update_latest_link(path)?;
    }
    Ok(())
}
...
fn report_site() -> ReportSite {
    ReportSite::new(repo_root()).with_fixture_pack_dir(repo_root().join("fixtures/packs"))
}
